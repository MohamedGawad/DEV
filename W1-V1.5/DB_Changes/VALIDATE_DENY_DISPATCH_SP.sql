create or replace PROCEDURE VALIDATE_DENY_DISPATCH_SP(INPUT_VAL IN XMLTYPE, STATUS_VAL OUT VARCHAR2) AS 
TRANS_DATE_VAL Date;
RECORD_ID NUMBER;
...
SELECT FI_DENY_SUMMARY_VALIDATION_ID, TRANS_DATE, INVOLVED_PARTY_ID, INVOLVED_PARTY_ID_TYPE, NAT_CD INTO RECORD_ID, TRANS_DATE_VAL, CUST_ID_VAL, CUST_ID_TYPE_VAL, NATIONALITY_CODE_VAL
FROM FI_DENY_SUMMARY_VALIDATION
WHERE FI_DENY_SUMMARY_VALIDATION.MSG_UID = r.MSGUID;
...
IF (STATUS_VAL = 'VALID')
THEN
DELETE FROM FI_DENY_SUMMARY_VALIDATION WHERE FI_DENY_SUMMARY_VALIDATION_ID = RECORD_ID;
